...
def forgot_password(request):
  ...
  if not email:
    return render(request, 'app/forgot.html', {'error': 'Por favor, insira o seu email.'})

  user = User.objects.filter(email=email).first()

  if not user:
    return render(request, 'app/forgot.html', {'error': 'O email inserido não está registado.'})
